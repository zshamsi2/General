...
class mkStarting_noMSM:
    """
    Finding the position of one frame in the desired cluster
    """
    def findElementPosition(self, l, desCluster):
        """
        """
        flag = False
        n_trajs = len(l)
    ...
    def populationFind(self, l, myn_clusters):
        """
    ...
    def writeOPF_lessPop(self, l,T, myn_clusters, n_samples):
        """
        The function is using other functions, find the less populated 
        clusters and write the information of the cluster and cluster 
        centers, their corresponding trajectory name, frames and other
        information into a dictionary file, "lessPopClsInf.txt".
        "T" is the name of trajectories array, in the same order as it
        was given into the msmbuilder, "l is the cluster labels 
        corresponding to each, "myn_clusters" is the number of clusters
        and "n_sample" is the number of samples which are desired for 
        the new round.
        """
        OPF = []
    ...
    def CpptrajInGen_commonTop(self, topFile):
        """
        ...
        def writeOPF_all(self, l,T, myn_clusters, n_samples):
        """
